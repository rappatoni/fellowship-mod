...
let rec linear_term id canoccur =
 function
  ...
  | LambdaFO (id',p,t) -> if id = id' then true else linear_term id canoccur t
  ...
  | Mu (id',p,cmd) -> if id = id' then true else linear_command id false cmd
and linear_context id canoccur =
 function
  ...
  | Lambda' (id',p,c) ->
     if id = id' then true else linear_context id false (*canoccur*) c
  | DestructTermsPair (id',p',id'',p'',cmd) ->
     if id = id' || id = id'' then true else linear_command id canoccur cmd
  | DestructTermsPairFO (id',p',c) ->
     if id = id' then true else linear_context id canoccur c
  | ContextsPair (c,c') ->
     linear_context id canoccur c && linear_context id canoccur c'
  | MuTilde (id',p,cmd) ->
     if id = id' then true else linear_command id canoccur cmd
