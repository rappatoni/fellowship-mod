...
(*For goals*)
let rec pretty_hyp frm h = fprintf frm "%s:%s@\n" (fst h) (pretty_prop (snd h))
let rec pretty_ccl frm c = fprintf frm "%s:%s@\n" (fst c) (pretty_prop (snd c))
...
let pretty_goal (id,goal) frm =
 ...
 match goal.active with
  | (RightHandSide,p) -> 
      ...
      fprintf frm "%s%s@\n" turnstile id ;
      pretty_ccls (("*",p)::goal.ccl) frm
  | (LeftHandSide,p) ->
      pretty_hyps (("*",p)::goal.hyp) frm ; 
      fprintf frm "%s%s@\n" turnstile id ;
