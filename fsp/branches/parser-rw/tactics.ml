...
object (self)
  ...
  method to_string =
    match msg with
      ...
      | ContractionNotAllowed ->
         "Contraction not allowed in intuitionistic logic."
      | Arg_count -> 
  ...
  (match args with
	...
	try if (snd g.active) = 
	 match fst g.active with 
	  RightHandSide -> (search x g.hyp thms) | LeftHandSide -> List.assoc x g.ccl
	then 
	 let pt' =
	  match (fst g.active) with
...
(*let interpret_tac localized_t m =
 let rec aux (t,loct) =
  let t' =
   match t with
    (* primitive tactics *)
    ...
    | Contraction ident -> contraction ident
  in
   t',loct
 in
  apply__tac (aux localized_t) m
*)
