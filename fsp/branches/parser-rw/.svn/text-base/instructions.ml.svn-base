...
let pretty_instruction = function
  ...
  | Qed  -> "qed"
  | Ack  -> "checkout"
  | AckProofTerm  -> "checkout proof term"
  | ExportNaturalLanguage -> "checkout natural natural"
  | Undo -> "undo"
  | Discard -> "discard"
  | Quit -> "quit"
...
let goal args cairn = match args, cairn with
  | [Labeled_prop ([name],prop)], Idle (_,s) -> 
      begin match prop_infer prop s.sign with
	 | Inl SProp -> 
...
let qed args cairn = match args, cairn with 
  ...
  | _, Success s when s.goals=[] -> Exception (new tactic_msg No_args,s)
...
let discard args cairn = match args, cairn with
  | [All], _ ->
      too_late := false;
      trace := [] ;
      history := [] ;
  ...
  | [Theorem], _ ->
	...
	goals=[]})
  | _, Idle (_,s) -> 
      Idle (Some (new tactic_msg (Need_args "what you want to discard")),s)
  | _, Success s | _, Subgoals (_,s) | _, Exception (_,s) ->
      Exception (new tactic_msg (Need_args "what you want to discard"),s)
