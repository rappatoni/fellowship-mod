...
let help_by_default =
  ("by default",
   "Mark the current open obligation as a delegation." ^^^
   "Operationally, the obligation stays in the same goal list and can still be" ^^
   "selected with next/prev and discharged by the usual tactics." ^^^
   "The visible difference is in the proof term and goal marker: ordinary open" ^^
   "obligations are printed as ?n, while delegations are printed as !n." ^^^
   "A delegated obligation still blocks qed until it is discharged.")
...
let jack_tac =
 function
  ...
  | Cut -> help_cut
  | Elim -> help_elim
  | ByDefault -> help_by_default
  | Idtac -> help_idtac
  | Focus -> help_focus
  | Elim_In -> help_elim_in
  | Contraction -> help_contraction
  | Weaken -> help_weaken
  | Moxia -> ("moxia [x]", "Try to discharge the current left-hand obligation using a named moxia or the first matching one.")

let tac_list =
 String.concat ", "
  (List.map pretty_tactic
    (* in alphabetical order *)
    [ Axiom; ByDefault; Contraction; Cut; Elim; Elim_In; Idtac; Focus; Moxia; Weaken ]
  ) ^ "."
...
let jack_instr =
 function
  ...
  | Deny -> ("deny x : P", "Declare the proposition P as a named moxia/anti-axiom x.")
  | AntiTheorem -> ("antitheorem x : P", "Starts a counterproof/refutation of P on the left-hand side.")
  | Next -> help_next
  | Prev -> help_prev
  | Qed -> help_qed
